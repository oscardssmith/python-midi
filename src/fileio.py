...
class FileReader(object):
    ...
    def parse_midi_event(self, trackdata):
        ...
        stsmsg = ord(six.advance_iterator(trackdata))
        # is the event a MetaEvent?
        if MetaEvent.is_event(stsmsg):
            cmd = ord(six.advance_iterator(trackdata))
            ...
            data = [ord(six.advance_iterator(trackdata)) for x in range(datalen)]
            return cls(tick=tick, data=data, metacommand=cmd)
        # is this event a Sysex Event?
        elif SysexEvent.is_event(stsmsg):
            data = []
            while True:
                datum = ord(six.advance_iterator(trackdata))
                if datum == 0xF7:
                    break
                data.append(datum)
        ...
        # not a Meta MIDI event or a Sysex event, must be a general message
            ...
            if key not in EventRegistry.Events:
                ...
                data.append(stsmsg)
                data += [ord(six.advance_iterator(trackdata)) for x in range(cls.length - 1)]
                return cls(tick=tick, channel=channel, data=data)
            else:
                self.RunningStatus = stsmsg
                cls = EventRegistry.Events[key]
                channel = self.RunningStatus & 0x0F
                data = [ord(six.advance_iterator(trackdata)) for x in range(cls.length)]
                return cls(tick=tick, channel=channel, data=data)
...
class FileWriter(object):
    ...
    def encode_midi_event(self, event):
        ...
        if isinstance(event, MetaEvent):
            ...
            ret += str.join('', six.map(chr, event.data))
        # is this event a Sysex Event?
...
def write_midifile(midifile, pattern):
    if type(midifile) in six.string_types:
